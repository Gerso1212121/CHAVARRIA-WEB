...
const DOMAIN_URL = 'https://carpinteriachavarria.com';
...
// 🔔 Webhook de Wompi (actualiza pago y genera pedido)
// 🔔 Webhook de Wompi
app.post('/webhook/wompi', async (req, res) => {
  ...
  if (evento === 'transaccion_exitosa' && referencia) {
    ...
    // 🔎 Obtener detalles del pedido con producto
...
async function procesarPedidoPorReferencia(referencia) {
  ...
  const { data: cliente } = await supabase.from('Cliente').select('*').eq('id_cliente', clienteId).single();
  ...
  if (cliente?.correo) {
    ...
    const itemsTexto = detalleList.map(item => `• Producto #${item.id_producto} x${item.cantidad} - $${item.subtotal.toFixed(2)}`).join('\n');
    ...
    try {
      ...
      console.log('📧 Correo de factura enviado a', cliente.correo);
    } catch (e) {

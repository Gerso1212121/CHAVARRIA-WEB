...
app.post('/generar-link', async (req, res) => {
  ...
  console.error('Error al generar el enlace:', e.response?.data || e.message);
  res.status(500).json({ error: 'Error al generar enlace de pago', detalle: e.response?.data || e.message });
}
});
